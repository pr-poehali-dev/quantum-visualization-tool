...
// Развёрнутые описания для карточки товара (модальное окно «Подробнее»).
// В коллекции на главной используется краткое описание из БД — оно не меняется.
const STORY_BY_ID: Record<number, string> = {
  1: "«Боярин» — стол с характером, достойным своего имени. Массив сращенного дуба тёплого тикового оттенка подчёркивает благородную текстуру дерева, а плавный подъёмный механизм на два мотора позволяет менять высоту столешницы от сидячего до постоянного положения одним нажатием — без рывков и лишнего шума. Встроенная защита от столкновений заботится о безопасности, пока стол незаметно подстраивается под ваш день. Строгий чёрный каркас и лаковое покрытие делают «Боярина» центром рабочего пространства, которое одинаково хорошо смотрится и в кабинете, и в гостиной.",
  2: "«Купец» — стол для тех, кто ценит лаконичность и практичность без компромиссов. Бесцветный лак оставляет натуральный рисунок сращенного дуба главным украшением столешницы, а компактные размеры делают стол удобным даже для небольших комнат. Подъёмный механизм с двумя моторами и плавным ходом превращает рутинную работу в комфорт: несколько секунд — и стол готов к тому, чтобы за ним поработать стоя или сидя. Надёжная защита от столкновений и лаконичный чёрный каркас — практичное решение на каждый день.",
  3: "«Воевода» — стол основательный и надёжный, как и подобает его имени. Стационарное металлическое подстолье с поперечным соединением создаёт крепкую, устойчивую конструкцию, которая простоит десятилетия без единого скрипа. Столешница из сращенного дуба, покрытая бесцветным лаком, сохраняет естественную красоту древесины — каждый стол по-своему уникален. Это выбор тех, кто предпочитает классику: никаких моторов и электроники, только массив дерева и продуманная до мелочей геометрия для комфортной работы за компьютером.",
  4: "«Витязь» — самый просторный стол в коллекции, созданный для тех, кому важен простор рабочей поверхности. Светлый бесцветный лак мягко подчёркивает благородную фактуру сращенного дуба, а белый каркас подъёмного механизма добавляет лёгкости и хорошо вписывается в светлые интерьеры. Два мотора обеспечивают плавную и бесшумную регулировку высоты, а система защиты от столкновений делает использование стола безопасным для всей семьи. «Витязь» — про свободу движения и пространство для больших задач.",
}
...
function toProject(p: Product): ProjectItem {
  ...
  return {
    ...
    story: STORY_BY_ID[p.id] || p.description || "",
    year: YEAR,
